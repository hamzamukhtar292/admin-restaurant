...
const LoginPage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-base">
      <h2 className="text-4xl font-bold text-textMain mb-8">Admin Panel</h2>
      <div className="w-96 p-8 shadow-lg rounded-lg bg-base border border-borderBase">
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            <label
              htmlFor="username"
              className="block  text-grey6 font-normal text-[14px] mb-2"
            >
              Username
            </label>
            <input
        ...
        {status === "pending" && (
          <p className="text-center mt-4">Logging in...</p>
        )}
        {status === "success" && (
